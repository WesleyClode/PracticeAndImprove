<template>
  <div id="app">
    <div>num: {{this.$store.state.num}}</div>
    <div>getNum: {{this.$store.getters.getNum}}</div>
...
export default {
  ...
  methods: {
    ...
    minus(){
      this.$store.commit("syncMinus", 5);
    },
    asyncAdd(){
      this.$store.dispatch("asyncAdd", 15);
    },
    syncItAdd(){
      this.$store.commit("syncAdd", 15);
    }
  }
}
</script>
